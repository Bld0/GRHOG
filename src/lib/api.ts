...
export const API_ENDPOINTS = {
  ...
  // Dashboard endpoints
  DASHBOARD_ACTIVE_BINS: buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.ACTIVE_BINS),
  DASHBOARD_ACTIVE_CARDS: buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.ACTIVE_CARDS),
  DASHBOARD_CURRENT_USAGE: buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.CURRENT_USAGE),
  DASHBOARD_AVERAGE_FILLING: buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.AVERAGE_BIN_FILLING),
  // Analytics endpoints
  ANALYTICS_BIN_STATISTICS: buildApiUrl(API_CONFIG.ENDPOINTS.ANALYTICS.BIN_STATISTICS),
  ANALYTICS_USAGE_STATISTICS: buildApiUrl(API_CONFIG.ENDPOINTS.ANALYTICS.USAGE_STATISTICS),
  ANALYTICS_PENETRATION_ANALYSIS: buildApiUrl(API_CONFIG.ENDPOINTS.ANALYTICS.PENETRATION_ANALYSIS),
  ANALYTICS_CLEARING_EFFICIENCY: buildApiUrl(API_CONFIG.ENDPOINTS.ANALYTICS.CLEARING_EFFICIENCY),
...
export async function signIn(credentials: LoginCredentials): Promise<AuthResponse> {
  const response = await apiClient.post<AuthResponse>(API_ENDPOINTS.AUTH_SIGNIN, credentials, false);
...
export async function signOut(): Promise<{ message: string }> {
  try {
    await apiClient.post<{ message: string }>(API_ENDPOINTS.AUTH_SIGNOUT, {}, true);
  } catch (error) {
...
export async function refreshToken(refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> {
  return apiClient.post<{ accessToken: string; refreshToken: string }>(API_ENDPOINTS.AUTH_REFRESH, { refreshToken }, false);
}
...
export async function getUsageStatistics(): Promise<UsageStatistics> {
  return apiClient.get<UsageStatistics>(API_ENDPOINTS.ANALYTICS_USAGE_STATISTICS);
}

export async function getPenetrationAnalysis(): Promise<PenetrationAnalysis> {
  return apiClient.get<PenetrationAnalysis>(API_ENDPOINTS.ANALYTICS_PENETRATION_ANALYSIS);
}

export async function getClearingEfficiency(): Promise<ClearingEfficiency> {
  return apiClient.get<ClearingEfficiency>(API_ENDPOINTS.ANALYTICS_CLEARING_EFFICIENCY);
}

// Client activity functions
export async function getClientActivity(cardId: string): Promise<ClientActivity> {
  return apiClient.get<ClientActivity>(`${API_ENDPOINTS.CLIENT_ACTIVITY}/${cardId}/activity`);
}

// Dashboard functions
export const getDashboardActiveBins = async (): Promise<DashboardActiveBins> => {
  return apiClient.get<DashboardActiveBins>(API_ENDPOINTS.DASHBOARD_ACTIVE_BINS);
};

export const getDashboardActiveCards = async (): Promise<DashboardActiveCards> => {
  return apiClient.get<DashboardActiveCards>(API_ENDPOINTS.DASHBOARD_ACTIVE_CARDS);
};

export const getDashboardCurrentUsage = async (): Promise<DashboardCurrentUsage> => {
  return apiClient.get<DashboardCurrentUsage>(API_ENDPOINTS.DASHBOARD_CURRENT_USAGE);
};

export const getDashboardAverageFilling = async (): Promise<DashboardAverageFilling> => {
  return apiClient.get<DashboardAverageFilling>(API_ENDPOINTS.DASHBOARD_AVERAGE_FILLING);
};

export const getCollectionTrends = async (): Promise<CollectionTrends> => {
  return apiClient.get<CollectionTrends>(buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.COLLECTION_TRENDS));
};

export const getBinStatusDistribution = async (): Promise<BinStatusDistributionData> => {
  return apiClient.get<BinStatusDistributionData>(buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.BIN_STATUS_DISTRIBUTION));
};

export const getDailyUsageTrend = async (): Promise<DailyUsageTrendData> => {
  return apiClient.get<DailyUsageTrendData>(buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.DAILY_USAGE_TREND));
};

export const getLocationStats = async (): Promise<LocationStatsData> => {
  return apiClient.get<LocationStatsData>(buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.LOCATION_STATS));
}; 
...
export const getKhorooList = async (district?: string): Promise<number[]> => {
  ...
  return apiClient.get<number[]>(buildApiUrl(`${API_CONFIG.ENDPOINTS.DASHBOARD.GET_KHOROO}?${params.toString()}`));
};

// Fetch client type counts filtered by district and optional khoroo
export const getClientTypeCounts = async (district?: string, khoroo?: number): Promise<Array<{ type: string; count: number }>> => {
  const params = new URLSearchParams();
  if (district) params.set('district', district);
  if (khoroo != null) params.set('khoroo', String(khoroo));
  return apiClient.get<Array<{ type: string; count: number }>>(buildApiUrl(`${API_CONFIG.ENDPOINTS.DASHBOARD.CLIENT_TYPE_COUNTS}?${params.toString()}`));
};

// Fetch all bins (used for map pins)
export const getAllBins = async (): Promise<Bin[]> => {
  return apiClient.get<Bin[]>(buildApiUrl(API_CONFIG.ENDPOINTS.DASHBOARD.ALL_BINS));
};
