  ...
  },
  {
    ...
    datasets: [
      ...
      },
      {
        ...
        description:
          'GET /api/raw-data/bin-message — bin_message хүснэгтийн түүхий мөрүүд',
      ...
      },
      {
        ...
        description:
          'GET /api/raw-data/bin-usage — bin_usage хүснэгтийн түүхий мөрүүд',
    ...
    datasets: [
      {
        ...
        description:
          'GET /api/raw-data/client — client хүснэгтийн түүхий мөрүүд',
  ...
  return (
    <Card>
      ...
      <CardContent>
        ...
        {status === 'error' && (
          <pre className='bg-muted max-h-[500px] overflow-auto rounded-lg p-4 text-xs whitespace-pre-wrap'>
            {JSON.stringify(data, null, 2)}
          </pre>
        )}
        {status === 'success' && isArray && (
          <pre className='bg-muted max-h-[600px] overflow-auto rounded-lg p-4 text-xs whitespace-pre-wrap'>
...
export default function RawDataPage() {
  ...
  const activeTabLoading = (
    TABS.find((t) => t.value === activeTab)?.datasets ?? []
  ).some((ds) => status[ds.key] === 'loading');
          ...
          {TABS.map((tab) => {
            const renderCard = (ds: Dataset) => (
              <DatasetCard
                ...
                onSearch={(value) =>
                  setSearch((prev) => ({ ...prev, [ds.key]: value }))
                }
                ...
                {tab.value === 'bin' ? (
                  <>
                    ...
                    <div className='grid grid-cols-1 items-start gap-6 lg:grid-cols-2'>
                      {tab.datasets
                        .filter(
                          (ds) => ds.key === 'bin' || ds.key === 'bin_usage'
                        )
                  ...
                  tab.datasets.map((ds) => renderCard(ds))
                )}
              </TabsContent>
            );
          })}
